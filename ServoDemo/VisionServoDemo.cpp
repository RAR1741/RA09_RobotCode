	...
	void Autonomous(void)
	{
		...
		*/		
		...
		while( IsAutonomous() )	
		{
			...
			if ( FindColor(mode, &td.hue, &td.saturation, &td.luminance, &par, &cReport) ){
				foundColor = true;
			...
			if(foundColor && !staleImage) {				
				...
				ShowActivity ("** %s found: Servo: x: %f  y: %f  increment: %f  y: %f  ", 
						td.name, horizontalDestination, verticalDestination, incrementH, incrementV);	
			} else  if (!staleImage) { 
				...
				if (panIncrement == 3) {
					...
					/* Vertical action: center the vertical after several loops searching */
