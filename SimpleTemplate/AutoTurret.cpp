...
void AutoTurret::InitTurret(int motorSlot, int motorChannel,
...
void AutoTurret::RegisterMasterControl(int *ptr)
{
	this->masterControl = ptr;
}
void AutoTurret::TurretControl(void)
{
	Auto();
}
void AutoTurret::TurretControl(Joystick * turretStick)
{
	switch (mode)
	{
...
void AutoTurret::Rotate(float input)
{
		EndServoish();
		pid->Disable();
		targetTrack->Disable();
...
void AutoTurret::ServoPositionMode(Joystick *turretStick)
{
	//pid->Enable();
#if 0
...
float AutoTurret::CurrentPosition(void)
{
	// Range: -1 to 1
	return Position_Encoder->GetVoltage();
}

float AutoTurret::EncoderVoltage(void)
{
	return Position_Encoder->GetVoltage(); 
}

void AutoTurret::ManualPositionMode(Joystick *turretStick)
{
	InitServoish();
...
void AutoTurret::AltTrack(void)
{
	...
	if (this->TargetInSight()) {
		...
		if (fabs(rel_pos_raw) < AutoTurret::kTargetAimpointThreshold)
		{
			// fire.
			TurnOnGoggles();
		}
...
void AutoTurret::TurnOnGoggles(void)
{
...
void AutoTurret::TurnOffGoggles()
{
	DriverStation *ds = DriverStation::GetInstance();
...
void AutoTurret::AltTrack(void)
{
	...
	if ( this->TargetInSight() ) {
		...
		float output = 1.0 / AutoTurret::kEncoderRange * error;
...
double AutoTurret::GetTarget_X()
{	
	return par.center_mass_x_normalized;
}

void AutoTurret::SetTurretPosition(float position)
{
#if USE_PID
...
void AutoTurret::AdjustServoPositions(float normDeltaHorizontal, float normDeltaVertical)	{
...
float AutoTurret::TurretPosition(void)
{
...
void AutoTurret::UpdateState(void)
{
#if 0
...
void AutoTurret::InitServoish(void)
{
#if USE_PID
	pid->Enable();
#endif
...
void AutoTurret::EndServoish(void)
{
#if USE_PID
	pid->Disable();
#endif
...
void AutoTurret::NotifySystem(void)
{
...
inline float AutoTurret::ServoToEncoderUnits(float servo)
{
	//return AutoTurret::kCWVoltage + 
	return AutoTurret::kCCWVoltage + (servo * AutoTurret::kEncoderRange);
	//return min_encoder_voltage + (servo * (max_encoder_voltage - min_encoder_voltage));
}

inline float AutoTurret::EncoderUnitsToServo(float volts)
{
	//return (volts - min_encoder_voltage) / (max_encoder_voltage - min_encoder_voltage);
	return (volts - AutoTurret::kCCWVoltage) / AutoTurret::kEncoderRange;
}
