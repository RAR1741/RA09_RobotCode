 ...
 * channels are allocated incorrectly. 
 * 
 * The full issue is not with the actual allocation, but with the 
 * allocation abstraction object (Resource, see Resource.[h|cpp]). The 
 * resource number is provided as a flattened row/column index, but the
 * flat index is computed incorrectly. The flat index should be
 * 
 * row * NUM_COLS + col -1
 * 
 * P. Frampton, 10/23/09
...
void AnalogChannel::InitChannel(UINT32 slot, UINT32 channel)
{
	...
	*/
	channels->Allocate(AnalogModule::SlotToIndex(slot) * kAnalogChannels + channel - 1);
	m_channel = channel;
